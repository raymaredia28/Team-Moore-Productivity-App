...
function getStreakDays(tasks: { completedAt: string | null }[]): number {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const completedOnDay = tasks.some(
      (t) => t.completedAt && t.completedAt.startsWith(day)
    )
...
export function ProgressView() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-stone-800">Your Progress</h1>
        <p className="text-stone-400 text-sm mt-0.5">A clear picture of what you've accomplished.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <StatCard
          label="Streak"
          value={`${streak}d`}
          sub={streak >= 3 ? "🔥 Nice streak!" : "Keep going!"}
        />
      </div>

      {/* 7-day bar chart */}
      <div className="bg-white rounded-2xl border border-stone-100 p-4">
        <p className="text-xs text-stone-400 uppercase tracking-wider font-medium mb-4">Last 7 days</p>
          ...
          {last7.map(({ label, done }) => (
            <div key={label} className="flex-1 flex flex-col items-center gap-1">
              <motion.div
                ...
                style={{ minHeight: done > 0 ? 4 : 2, backgroundColor: done === 0 ? '#e7e5e4' : undefined }}
              />
      ...
      {categoryBreakdown.length > 0 && (
        <div className="bg-white rounded-2xl border border-stone-100 p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wider font-medium mb-3">By category</p>
